...
public class VoteService {
	...
	public void voteItemInsert(VoteDTO vdto, List<String> vote_item_names) {
		Long vote_id = voteMapper.findVoteId(vdto.getTeam_board_id());
		System.out.println("이건뭐냐?????????"+vote_id);
		if (vote_id != null && vote_item_names.size() >= 2) {
			System.out.println("11111111111111111111111111111");
			for (String vote_item_name : vote_item_names) {
				System.out.println("2222222222222222222222222222222222222");
				voteMapper.voteItemInsert(vote_id, vote_item_name);
			}
		}
	}
